<template>
    <div class="min-h-screen bg-pattern flex flex-col items-center justify-center text-center px-4 py-20 text-slate-800">
      <!-- Título -->
      <h2 class="section-title mb-6">🅰️ Aprendamos el Abecedario</h2>
  
      <!-- Tarjeta de letra -->
      <div class="card max-w-md w-full">
        <h1 class="text-7xl font-bold mb-4 text-purple-700">{{ letraActual.letra }}</h1>
      ...
      <div class="btn-group mt-10">
        ...
        <button
          @click="siguiente"
          :disabled="indice === letrasFiltradas.length - 1"
          class="cta-button"
        >
          Siguiente ➡️
        </button>
  ...
  import { abecedario } from '../data/abecedario.js'
  ...
  const ordenAymara = [
    "A", "Ch", "Chh", "E", "H", "I", "J", "K", "Kh", "L",
    "Ll", "M", "N", "Ñ", "O", "P", "Ph", "Q", "Qh", "R",
    "S", "T", "Th", "U", "W", "X", "Y"
  ]
  
  const letrasFiltradas = computed(() => {
    const disponibles = abecedario.filter(letra =>
      letra.disponibleEn.includes(idioma)
    )
  
    if (idioma === 'ay') {
      return ordenAymara
        .map(letra => disponibles.find(l => l.letra === letra))
        .filter(Boolean)
    }
  ...
  const indice = ref(0)
  const letraActual = computed(() => letrasFiltradas.value[indice.value])
  const contenido = computed(() => letraActual.value.idiomas[idioma])
  
  function siguiente() {
    if (indice.value < letrasFiltradas.value.length - 1) indice.value++
  }
  ...
  function reproducirAudio() {
    const path = `/audio/letras/${letraActual.value.letra.toLowerCase()}-${idioma}.mp3`
